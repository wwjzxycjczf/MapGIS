...
netnodelist="{\"type\":\"netnodelist\", \"data\":[{\"Nodeid\":\"1\",\"Iphostname\":\"192.168.1.166\",\"Ipaddr\":\"192.168.1.166\",\"ISON\":\"true\"} ]}";
var markers=[];
var ndmarkers =[];
//radarlist="sss";
...
//	RShow();
//	netnodelist = JSON.parse(netnodelist);
    if (SocketCreated) {
        SocketCreated = false;
        ws.close();
    } else {
...
//    	if (stringToJson(netnodelist) != datas) {
	//    	deleteAllndMarkers();
	   // 	deleteAllul();
	        netnodelist = datas;
	        for(var i=0;i<netnodelist.data.length;i++){
	        	var state="";
	    	if(netnodelist.data[i].ISON=="true"){
	    		state = "在线";
	    	}else if(netnodelist.data[i].ISON=="false"){
	    		state ="离线";
	    	}
	    	Log("Nodeid:"+netnodelist.data[i].Nodeid+"   主机状态:"+state);
//	        }
//	        RShow(2);
   //     Addul();
		}  
...
//				}
	...
	if(inx==len){
		...
		netnodelist.data.push(data.node);
...
function changeRadar(data) {
	...
	if(data.radar.is_on=="1"){
		is_on = "关机";
	}
	Log("RadaID:"+data.radar.RadaID+"   状态:"+state+" "+is_on);
...
//	ul.childNodes[liindex].style.backgroundColor = "";
		...
		for (inx = 0; inx < len; inx++) {
			if(data.radar.RadaID==radarlist.data[inx].RadaID){
				...
				if(data.radar.is_on=="1"){
					...
					document.getElementById("alertinfo").innerHTML = "雷达"+data.radar.RadaID+"关机了";
					document.getElementById("alertinfo").style.display = "block";
					setTimeout("alertdisappear()",1000);
				}else{
					if(showallflag==true
	...
	if(inx==len){
		...
		radarlist.data.push(data.radar);
	 ...
	 if(flag==0){
		for(var i=0;i<radarlist.data.length;i++){
		 ...
		 for(var i=0;i<netnodelist.data.length;i++){
		        ...
		        table.appendChild(_row);  
		        _cell = document.createElement("td"); 
		        _cell.innerText = "netID:"+netnodelist.data[i].Nodeid+"    主机名:"+netnodelist.data[i].Iphostname+"    主机IP:"+netnodelist.data[i].Ipaddr
