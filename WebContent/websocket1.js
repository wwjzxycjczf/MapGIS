...
function changeNetnode(data){
	var state="";
	if(data.node.ISON==true){
		state = "在线";
	}else if(data.node.ISON==false){
		state ="离线";
	}
...
//	ul.childNodes[liindex].style.backgroundColor = "";
		...
		for (inx = 0; inx < len; inx++) {
			if(data.radar.RadaID==radarlist.data[inx].RadaID){
				...
				if(showallflag==true||((showallflag==false)&&((data.radar.state=="1"&&onshowflag)||(data.radar.state=="2"&&silentshowflag)||
...
function RShow(flag) {
	var len=0;
	if(flag==1){
		len = radarlist.data.length;
	}else if(flag==2){
		len = netnodelist.data.length;
	}
	if(showallflag){
	for (var i = 0; i < len; i++) {
       		drawMarker(i,flag);
...
function drawMarker(i,flag){
	if(flag==1){
		...
		var infostr = "Nodeid:"+netnodelist.data[i].Nodeid+';\n'+"主机名:"+netnodelist.data[i].Iphostname+";\n"+
	  	"主机ip地址:"+netnodelist.data[i].Ipaddr+";\n"+"主机状态:"+netnodelist.data[i].ISON+";";
		var marker = new google.maps.Marker({
	   		map:map,
	   		title:infostr,
	   		position:new google.maps.LatLng(netnodelist.data[i].lat,netnodelist.data[i].lon)
	   	});
		 ndmarkers.push(marker);
//		google.maps.event.addListener(ndmarkers[i],'click',RadarClick);
	}
}
	 ...
	 if(flag==0){
		for(var i=0;i<radarlist.data.length;i++){
		 ...
		 for(var i=0;i<networkdevicelist.data.length;i++){
		        ...
		        table.appendChild(_row);  
		        _cell = document.createElement("td"); 
		        _cell.innerText = "Nodeid:"+networkdevicelist.data[i].Nodeid+"    主机名:"+networkdevicelist.data[i].Iphostname+"    主机ip地址:"+networkdevicelist.data[i].Ipaddr
		        +"    状态:"+networkdevicelist.data[i].ISON;
			    _row.appendChild(_cell);
//		        _row.onclick = rowClick;
//		        _row.onmouseover = rowOver;
//		        _row.onmouseout = rowOut;
